...
(values
  ('arraigo', '¿Te suena alguna de estas?', null, 'single', '[{"value":"volvio","label":"Me fui del país y volví"},{"value":"se-quedo","label":"Nunca me fui de Venezuela"},{"value":"interior","label":"Llegué de otra ciudad y no conozco a nadie"},{"value":"visita","label":"Estoy de paso"},{"value":"mismos","label":"Sigo con la gente de siempre"},{"value":"remoto","label":"Trabajo remoto y casi no veo gente"}]'::jsonb, null, null, true, true, null, null, null, 1, 10),
...
delete from answers a
 ...
 where a.version_id = v.id and v.version = 'v3'
   and a.question_key not in ('arraigo', 'sector', 'empleador', 'momento', 'rol', 'motivo', 'romance', 'temas', 'evitar', 'actividades', 'planes', 'peso', 'gasto', 'zonas', 'dias', 'idiomas', 'dieta');

delete from questions q
 using questionnaire_versions v
 where q.version_id = v.id and v.version = 'v3'
   and q.key not in ('arraigo', 'sector', 'empleador', 'momento', 'rol', 'motivo', 'romance', 'temas', 'evitar', 'actividades', 'planes', 'peso', 'gasto', 'zonas', 'dias', 'idiomas', 'dieta');

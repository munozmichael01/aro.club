-- Los rasgos de una cuenta dada de baja volvian solos.
--
-- `dar_de_baja` los borra, pero luego anonimiza el perfil y eso dispara el
-- trigger de `profiles`, que llama a `refrescar_rasgos` y vuelve a crear la
-- fila —vacia, pero ahi—. Reordenar los pasos dentro de la funcion no lo
-- arregla: borrar las respuestas dispara el otro trigger y pasa lo mismo.
--
-- El arreglo va en la invariante, no en el orden: **una cuenta dada de baja
-- no tiene rasgos**, la toque quien la toque.
--
-- El cuerpo es el de la migracion que la creo, copiado tal cual. Solo se
-- añade la guarda de arriba.
